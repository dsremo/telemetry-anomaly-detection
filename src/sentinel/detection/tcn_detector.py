...
class TCNDetector:
    ...
    def save(self, path: Path) -> None:
        ...
        Saves a small checkpoint (~25 KB) with the PyTorch state_dict plus
        ...
        try:
            ...
            logger.debug("tcn_model_saved", path=str(path))
        except Exception as exc:
            logger.warning("tcn_model_save_failed", path=str(path), error=str(exc))
    ...
    def load(self, path: Path) -> bool:
        ...
        try:
            ...
            self._is_fitted       = True
            self._samples_since_fit = 0
            logger.debug("tcn_model_loaded", path=str(path))
            return True
        except Exception as exc:
            logger.warning("tcn_model_load_failed", path=str(path), error=str(exc))
            return False
