...
from sentinel.api.schemas import ApiKeyCreateRequest, ApiKeyCreateResponse, ApiKeyOut
...
async def create_api_key(
    ...
    if not tenant_id:
        raise HTTPException(
            status_code=400,
            detail="X-Tenant-ID header required when operating as a sentinel user",
        )
