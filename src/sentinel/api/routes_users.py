...
RBAC:
  All routes require at minimum tenant_admin level (admin, superuser, sentinel_admin).
...
def _max_assignable_tier(requester_role: str) -> int:
    ...
    Sentinel users (any sentinel role) can assign any tenant role (up to tier 4).
    Tenant admins can assign up to their own tier.
    """
    if requester_role in _SENTINEL_ROLES:
        return 4
